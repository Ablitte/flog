...
func (logger *Logger) GenDayFile(now *time.Time) error {
	...
	filename := fmt.Sprintf("%d年%02d月%02d日%02d时.log",
		now.Year(),
		now.Month(),
		now.Day(),
		now.Hour())
	...
	if logger.filePath != "" {
		var err error
		os.MkdirAll(logger.filePath, os.ModePerm)
		logger.outFile, err = os.OpenFile(path.Join(logger.filePath, logger.filepre+filename), os.O_APPEND|os.O_CREATE|os.O_RDWR, os.ModePerm)
		if err != nil {
			return err
		}
...
func New(strLevel string, pathName string, filePre string, buffNum uint32) (*Logger, error) {
	...
	logger.flag = log.LstdFlags | log.Lshortfile
...
func init() {
	gLogger, _ = New("debug", "logs", "log_", 1)
}
...
func SetLogPath(path string, filePrefix string, showConsole bool) {
	gLogger.filePath = path
	gLogger.filepre = filePrefix
